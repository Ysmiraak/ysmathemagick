(ns ysmathemagick.statistics
  ...
  (:require [clojure.spec :as s]
            [incanter.stats :as sta]))

(s/def ::fin #(Double/isFinite %))
...
(s/fdef fake-sample
        :args (s/cat :size (s/and pos-int? #(<= 2 %))
                     :mean (s/and number? ::fin)
                     :sd (s/and number? ::fin (comp not neg?)))
        :ret vector?
...
;;         )
;; (defn v->p
;;   "calculates the p-value from some other value v in distribution ∈
;;   {:t, :norm, :chi-sq}.
;;   " [distribution v
